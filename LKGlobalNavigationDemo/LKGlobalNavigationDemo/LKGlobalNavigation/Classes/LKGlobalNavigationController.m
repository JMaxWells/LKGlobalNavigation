...
+ (UIViewController *)findAnExistViewControllerWithURLPattern:(NSString *)URLPattern{
    for (NSInteger i = [LKGlobalNavigationController sharedInstance].viewControllers.count; i > 0; i--) {
        UIViewController *vc = [[LKGlobalNavigationController sharedInstance].viewControllers objectAtIndex:(i - 1)];
        Class vClass = [LKGlobalNavigationController findViewControllerClassWithURLPattern:URLPattern];
        if ([vc isKindOfClass:vClass]) {
            return vc;
        }
    }
    return nil;
...
+ (void)removeViewControllerWithURLPattern:(NSString *)URLPattern {
    ...
    NSMutableArray *viewControllers = [[NSMutableArray alloc] initWithArray:[LKGlobalNavigationController sharedInstance].viewControllers];

    if (!vc || ![viewControllers containsObject:vc]) {
        return;
    }

    [viewControllers removeObject:vc];
    [[LKGlobalNavigationController sharedInstance] setViewControllers:viewControllers];
}
